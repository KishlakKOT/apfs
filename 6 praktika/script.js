...
function generateGameLegend(playerName) {
  ...
  ${playerName} был готов ко всем испытаниям, которые ему предстояли на его пути. Его путь был долгим и полным опасностей - он встретил множество врагов, которые хотели остановить его в поисках артефакта. ${playerName} был настолько смелым и умелым воином, что справлялся с каждым врагом на своем пути. Однажды он встретил злого короля, который хранил артефакт. В битве с королем ${playerName} победил и вернул артефакт, и теперь он должен использовать его, чтобы уничтожить всех злых демонов и вернуть мир к прежнему состоянию. Теперь ${playerName} отправляется в последнее путешествие, чтобы уничтожить демонов и спасти мир. Его здоровье и богатство помогут ему в этом путешествии, но ему также придется принимать трудные решения и сражаться в жестоких битвах. Сможет ли ${playerName} победить всех демонов и вернуть мир к прежнему состоянию? Только время покажет.`;
}
...
function welcomePlayer() {
    ...
    while (!nameReceived) {
        ...
        if (name === "" || name === null) {
            alert("Ты не ввёл имя, попробуй ещё раз");
        } else {
